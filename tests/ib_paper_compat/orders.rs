                    ...
                    OrderStatus::Submitted => {
                        if modify_sent && !modify_acked {
                            ...
                            control_tx.send(ControlCommand::Order(OrderRequest::Modify {
                                order_id, new_order_id, price: 2_00_000_000, qty: 1, ord_type: 0, tif: 0, stop_price: 0, outside_rth: None })).unwrap();
                            modify_sent = true;
                        }
                    }
                ...
                match update.status {
                    OrderStatus::Submitted => {
                        if modify_sent && !modify_acked_local {
                            ...
                            order_acked = true;
                            control_tx.send(ControlCommand::Order(OrderRequest::Modify {
                                order_id, new_order_id, price: 1_00_000_000, qty: 2, ord_type: 0, tif: 0, stop_price: 0, outside_rth: None })).unwrap();
                            modify_sent = true;
                        }
                    }
                    OrderStatus::Cancelled => { order_cancelled = true; break; }
                ...
                match update.status {
                    OrderStatus::Submitted => {
                        if modify_sent && !modify_acked {
                            ...
                            control_tx.send(ControlCommand::Order(OrderRequest::Modify {
                                order_id, new_order_id, price: 2_00_000_000, qty: 3, ord_type: 0, tif: 0, stop_price: 0, outside_rth: None })).unwrap();
                            modify_sent = true;
                        }
                    }
                    OrderStatus::Cancelled => { order_cancelled = true; break; }
                    ...
                    OrderStatus::Submitted => {
                        match phase {
                            0 => {
                                ...
                                control_tx.send(ControlCommand::Order(OrderRequest::Modify {
                                    order_id, new_order_id: modify_id_1, price: 2_00_000_000, qty: 1, ord_type: 0, tif: 0, stop_price: 0, outside_rth: None })).unwrap();
                                phase = 1;
                            }
                            1 => {
                                // First modify acked → modify again to $3
                                control_tx.send(ControlCommand::Order(OrderRequest::Modify {
                                    order_id: modify_id_1, new_order_id: modify_id_2, price: 3_00_000_000, qty: 1, ord_type: 0, tif: 0, stop_price: 0, outside_rth: None })).unwrap();
                                phase = 2;
                            }
                            2 => {
