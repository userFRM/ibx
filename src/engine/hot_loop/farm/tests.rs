...
mod decode_publish_tests {
    ...
    /// A chargeable snapshot says when it was answered.
    ///
    /// The venue sends no such figure. The reference terminal reads its own
    /// clock where the snapshot's payload reaches it and before any of that
    /// payload is decoded, so the number means when the answer arrived rather
    /// than when any price in it was made. Once per snapshot, and only for a
    /// slot the snapshot holds: a stream is not answered, it runs.
    #[test]
    fn a_chargeable_snapshot_says_when_it_was_answered() {
        let mut farm = FarmState::new();
        let mut context = Context::new();
        let shared = SharedState::new();
        ...
        // A stream, which is not answered at a moment.
        farm.instrument_md_reqs.push((id, super::MdReqRecord {
            con_id: 756733, sec_type: String::new(), mode_9887: 0,
            entries: vec![super::MdReqEntry {
                req_id: 1, request_type: super::REALTIME_BID_ASK_REQUEST_TYPE,
                venue: String::new(),
            }],
        }));
        farm.handle_tick_data(
            &framed_35p(9, &[(tick_decoder::O_LAST_PRICE, 2, 501)]),
            &mut context, &shared, &None,
        );
        assert!(
            !shared.market.drain_series_ticks(id).iter().any(|t| t.tick_type == 85),
            "a stream is not stamped",
        );

        // And the snapshot, which is.
        farm.instrument_md_reqs.clear();
        farm.instrument_md_reqs.push((id, super::MdReqRecord {
            con_id: 756733, sec_type: String::new(), mode_9887: 0,
            entries: vec![super::MdReqEntry {
                req_id: 2, request_type: super::REGULATORY_SNAPSHOT_REQUEST_TYPE,
                venue: String::new(),
            }],
        }));
        farm.handle_tick_data(
            &framed_35p(9, &[(tick_decoder::O_LAST_PRICE, 2, 502)]),
            &mut context, &shared, &None,
        );
        let said = shared.market.drain_series_ticks(id);
        let stamp = said.iter().find(|t| t.tick_type == 85).expect("stamped");
        let millis: u128 = match &stamp.value {
            crate::types::SeriesValue::Text(t) => t.parse().expect("a number of milliseconds"),
            other => panic!("the moment is stated as text: {other:?}"),
        };
        assert!(millis > 1_600_000_000_000, "and it is a moment, not a count: {millis}");

        // Once. The rest of the snapshot's payload does not restate it.
        farm.handle_tick_data(
            &framed_35p(9, &[(tick_decoder::O_CLOSE_PRICE, 2, 503)]),
            &mut context, &shared, &None,
        );
        assert!(
            !shared.market.drain_series_ticks(id).iter().any(|t| t.tick_type == 85),
            "the moment is the snapshot's, not every message in it",
        );
    }
