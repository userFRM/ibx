...
/// A volatility order is taken as the caller states it: the volatility is
/// the order's price. Stated without one there is nothing to send, and the
/// order is refused rather than sent malformed.
#[test]
fn a_volatility_order_is_built_from_the_volatility_it_states() {
    let scale = crate::types::PRICE_SCALE;
    let order = ApiOrder {
        action: "BUY".into(),
        total_quantity: 100.0,
    ...
    let cmd = ClientCore::build_order_request(&order, 7, 0, None).unwrap();
    let ControlCommand::Order(OrderRequest::SubmitEx { kind, .. }) = cmd else {
        panic!("a volatility order routes through the shared extended submission");
    };
    let crate::types::OrderKind::Vol { volatility } = kind else {
        panic!("built as the wrong kind: {kind:?}");
    };
    assert_eq!(volatility, 40 * scale, "the volatility the caller stated");

    let unpriced = ApiOrder { volatility: f64::MAX, ..order.clone() };
    let err = ClientCore::validate_order(&unpriced, "DU123").unwrap_err();
    assert!(err.contains("volatility"), "a volatility order with no volatility is refused: {err}");
}

/// A relative combination that extends the limit type is taken as the caller
/// states it: the price on the limit-price field. Stated without one there
/// is nothing to send, and the order is refused rather than sent malformed.
#[test]
fn a_relative_limit_combination_is_built_from_the_price_it_states() {
    let scale = crate::types::PRICE_SCALE;
    let order = ApiOrder {
        action: "BUY".into(),
        total_quantity: 100.0,
        order_type: "REL + LMT".into(),
        lmt_price: 150.0,
        tif: "DAY".into(),
        ..Default::default()
    };
    ...
    let cmd = ClientCore::build_order_request(&order, 7, 0, None).unwrap();
    let ControlCommand::Order(OrderRequest::SubmitEx { kind, .. }) = cmd else {
        panic!("a relative limit combination routes through the shared extended submission");
    };
    let crate::types::OrderKind::RelLmt { price } = kind else {
        panic!("built as the wrong kind: {kind:?}");
    };
    assert_eq!(price, 150 * scale, "the price the caller stated");
    ...
    let unpriced = ApiOrder { lmt_price: 0.0, ..order };
    let err = ClientCore::validate_order(&unpriced, "DU123").unwrap_err();
    assert!(err.contains("lmt_price"), "a relative limit combination with no price is refused: {err}");
}

/// A relative combination that finishes as a market order is taken as the
/// caller states it: with no price of its own.
#[test]
fn a_relative_market_combination_is_built_without_a_price() {
    let order = ApiOrder {
        action: "BUY".into(),
        total_quantity: 100.0,
        order_type: "REL + MKT".into(),
        tif: "DAY".into(),
        ..Default::default()
    };
    ClientCore::validate_order(&order, "DU123").unwrap();
    let cmd = ClientCore::build_order_request(&order, 7, 0, None).unwrap();
    let ControlCommand::Order(OrderRequest::SubmitEx { kind, .. }) = cmd else {
        panic!("a relative market combination routes through the shared extended submission");
    };
    assert!(
        matches!(kind, crate::types::OrderKind::RelMkt),
        "built as the wrong kind: {kind:?}",
    );
}
