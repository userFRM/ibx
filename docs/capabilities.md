...
| | |
| --- | --- |
| Requests | 79. Every one either does what it says or reports why it cannot — none returns success having sent nothing |
...
| Tests | 2,280 offline, and 169 more that live in the suites run against a broker session |
...
| `ibx::Client` (Rust) | ✅ Supported | 79/79 callable; 3 return an error naming a local-process facility this client does not have |
...
| --- | --- |
| A call never returns success having sent nothing | 79 requests, none silent |
